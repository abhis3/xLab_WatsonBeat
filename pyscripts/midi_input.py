...
class RecordMidi :
    ...
    def __init__ ( self, dirname, tempo ) :
        ...
        self.tsInfo = { 'tsNumerator': 4, 'tsDenominator': 4, 'measureLength': 1920, 'resolution': 480, 'format':0 , 'bpm': tempo }
        ...
        try:
            ...
            with mido.open_input() as port:
                ...
                for message in port:
                    ...
                    if ( initialNotePlayed and message.time - self.initialClk >= self.secondsForPhraseLength )  :
    ...
    def createMidi ( self ) :
        ...
        print ( call )
        os.system ( call ) ;
        call = "mididump.py midi_export.mid > midi_export.txt"
        os.system ( call ) ;
        retval = os.getcwd()
        print ( "retval: ", retval )
        rmCmd = "rm -rf " + self.dirname + "/midiDump/midi_export.txt"
        os.system ( rmCmd ) ;
        rmCmd = "rm -rf " + self.dirname + "/midiDump/midi_export*"
        os.system ( rmCmd ) ;

        mvCmd = "mv midi_export.txt " + self.dirname+ "/midiDump/"
        os.system ( mvCmd ) ;
        mvCmd = "mv midi_export.mid " + self.dirname + "/midiDump/"
        os.system ( mvCmd ) ;
        mvCmd = "mv midi_export.py " + self.dirname + "/midiDump/"
        os.system ( mvCmd ) ;



        # os.sytem ( "open http://arlab053.austin.ibm.com:9000/public/index.html" )
        # cmd = '/usr/bin/open -a "/Applications/Google Chrome.app" 'http://google.com/'
...
if __name__ == '__main__' :
    ...
    midi = RecordMidi(dirname, tempo)
    midi.run()


    dataToSendBack = "New Data"
